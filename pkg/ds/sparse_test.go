...
func TestSparseList(t *testing.T) {
	...
	tests := []struct {
		...
		{ 
			...
			func (l *SparseList[string]) {
				...
				l.Add("5")
				l.Free(b);
				l.Compress(func (int, int, *string) {});
			},
			Expected{
				[]string{"1", "5"},
				[]int{},
	...
	for _, test := range tests {
		list := NewSparseList[string](10, 5)
		...
		if (len(list.free) != len(test.expected.free)) {
			t.Errorf("Test '%s' actual free(%d) and expected free(%d) are different lengths", test.name, len(list.free), len(test.expected.free));
		} else {
			for i, actualFree := range list.free {
				if (actualFree != test.expected.free[i]) {
					t.Errorf("Test '%s' free at index %d failed, %d <> %d", test.name, i, actualFree, test.expected.free[i]);
				}
			}
		}
	}
